...
void Grafo<T>::writeToDisk() {
  std::ofstream myFileV;
  std::ofstream myFileA;
  std::string fileNameV("file01.txt");
  std::string fileNameA("file02.txt");

  myFileV.open(fileNameV, std::ios_base::trunc);
  myFileA.open(fileNameA, std::ios_base::trunc);

  if(!myFileV.is_open()) {
    throw Exception("No se pudo abrir " + fileNameV + " para escritura");
    }

  if(!myFileA.is_open()) {
    throw Exception("No se pudo abrir " + fileNameA + " para escritura");
    }
...
void Grafo<T>::readFromDisk() {
  std::ifstream myFileV;
  std::ifstream myFileA;
  std::string fileNameV("file01.txt");
  std::string fileNameA("file02.txt");

  myFileV.open(fileNameV, std::ios::in);
  myFileA.open(fileNameA, std::ios::in);

  if(!myFileV.is_open()) {
    throw Exception("No se pudo abrir " + fileNameV + " para lectura");
    }

  if(!myFileA.is_open()) {
    throw Exception("No se pudo abrir " + fileNameA + " para lectura");
    }
  ...
  while((myFileV >> myObjet) and myFileV.ignore()) {
    insertVer(myObjet);
    }

  while((myFileA >> myObjet) and myFileA.ignore()) {
    myFileA >> myObjet2;
    myFileA.ignore();
